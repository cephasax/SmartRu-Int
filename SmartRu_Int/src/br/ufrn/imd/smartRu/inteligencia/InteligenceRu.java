...
public class InteligenceRu {
	...
	public void adicionarRaspberryFromJason(String json) {
		...
		System.out.println("Raspberry" + rasp.getIdentificador()
	...
	public int[][] receberMatriz() {
		int[][] matriz = new int[appFila.getNumeroLinhas()][appFila
				.getNumeroColunas()];

		/**
		 * Percorre todos os Raspberrys cadastrados por ordem de cadastro.
		 * Inclui como sensor da fila todos os sensores do raspberry atual ate
		 * que nao haja mais nenhum sensor. Apos isso segue para o proximo
		 * raspberry
		 * 
		...
		for (int i = 0; i < appFila.getNumeroLinhas(); i++) {
			for (int j = 0; j < appFila.getNumeroColunas(); j++) {
				...
				if ((i < appFila.getRasps().size())
					...
					v = appFila.getRasps().get(i).getSensores().get(j)
							.getValor();
				}
	...
	public double percentFila(int mat[][]) {
		int linha = appFila.getNumeroLinhas(), coluna = appFila
				.getNumeroColunas();
